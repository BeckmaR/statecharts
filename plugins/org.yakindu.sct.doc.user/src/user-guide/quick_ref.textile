...
| <code>operation doStuff()</code> | Operation, connects a state machine to the outside world by making external behaviour accessible. Implementation needs to be provided by the client code. |
| <code>operation average(a : real, b : real) : real</code> | Operation with two parameters a and b of type real and return type real. |
...
A transition reaction can specify the following *triggers* (or none):
