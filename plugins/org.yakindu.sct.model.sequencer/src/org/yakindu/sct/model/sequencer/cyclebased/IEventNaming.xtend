/** 
 * Copyright (c) 2019 itemis AG - All rights Reserved
 * Unauthorized copying of this file, via any medium is strictly prohibited
 * Contributors:
 * itemis AG
 */
package org.yakindu.sct.model.sequencer.cyclebased

import com.google.inject.ImplementedBy

@ImplementedBy(DefaultImpl)
interface IEventNaming {
	def String nameEventRaiser(CharSequence s)

	def String nameEventRaised(CharSequence s)

	def String nameEventValue(CharSequence s)

	def String nameEventValueGetter(CharSequence s)

	def String nameEventIsRaised(CharSequence s)

	static class DefaultImpl implements IEventNaming {
		override nameEventRaised(CharSequence s) {
			s + "Raised"
		}
		
		override nameEventRaiser(CharSequence s) {
			"raise" + s.toString.toFirstUpper
		}
		
		override nameEventIsRaised(CharSequence s) {
			"isRaised"+s.toString.toFirstUpper
		}
		
		override nameEventValue(CharSequence s) {
			return '''«s»Value'''
		}
		
		override nameEventValueGetter(CharSequence s) {
			return '''get«s»Value'''
		}
	}
}
